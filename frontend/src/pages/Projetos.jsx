...
const Projetos = () => {
  ...
  useEffect(() => {
    const fetchProjetos = async () => {
      setLoading(true);
      try {
        const response = await fetch('/api/projetos?populate=*', {
        ...
        // CORRIGIDO: Filtrar por 'titulo' em vez de 'nome'.
        const validProjetos = (jsonResponse.data || []).filter(projeto => {
          if (!projeto || !projeto.documentId || !projeto.titulo) {
            console.warn('Projeto inválido da API (sem documentId ou titulo) foi ignorado:', projeto);
            return false;
          }
          return true;
        });
        ...
        {!loading && !error && (
          <motion.div className={viewMode === 'grid' ? "grid md:grid-cols-2 lg:grid-cols-3 gap-10" : "space-y-6"} variants={containerVariants} initial="hidden" animate="visible">
            {projetos.map((projeto) => {
              const imageUrl = projeto.imagem_destaque?.url ? `${STRAPI_BASE_URL}${projeto.imagem_destaque.url}` : 'https://via.placeholder.com/400x300';
              const plainDescription = Array.isArray(projeto.descricao) ? projeto.descricao.map(b => b.children.map(c => c.text).join('')).join(' ') : projeto.descricao;

              return (
                <motion.div key={projeto.documentId} variants={itemVariants}>
                  <Link to={`/projetos/${projeto.documentId}`} className="block h-full">
